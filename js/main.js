// Global variables
let w, h, c, ctx, activeWordList, wordCount, danger, points, multiplier, dangerColor, addNewWord, seconds, pointNotes, lives, timer, processClock, accuracy, missedWordCount, stopFunction, endGame, maxWPM, mmProcess, singlePlayerSelected, multiPlayerSelected, shopSelected, title;
...
function handleMenuInput(e) {
    let enter = 13,
        ...
        sBuff = unselected;
    // If up arrow and sp
    if (e.keyCode === u && singlePlayerSelected) {
        // nothing atm
    }

    // If up arrow and mp
    if (e.keyCode === u && multiPlayerSelected) {
        console.log('re');
    ...
    if (e.keyCode === d && singlePlayerSelected) {
        console.log('re');
        let menuSwitch = setInterval(function() {
            if (usBuff !== unselected && sBuff !== selected) {
    ...
    if (e.keyCode === d && multiPlayerSelected) {
        console.log('re');
