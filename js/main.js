// Global variables
let w, h, c, ctx, activeWordList, wordCount, danger, points, multiplier, dangerColor, addNewWord, seconds, pointNotes, lives, timer, processClock, accuracy, missedWordCount, endGame;
...
function preEndGameProcess() {
    // Create pointNotes for all words active
    for(let i = 0; i < activeWordList.length; i++) {
        pointNotes.push(activeWordList[i]);
        activeWordList.pop(i);
        console.log("ac: " + activeWordList.length);
    }

    // Send the words floating up like ghosts or something
    for (let i = 0; i < pointNotes.length; i++) {
        ctx.fillStyle = "#604767";
        ctx.globalAlpha = pointNotes[i].alpha;
        ctx.fillText(pointNotes[i].text, pointNotes[i].x, pointNotes[i].y);
        pointNotes[i].y-=3;
        pointNotes[i].alpha = pointNotes[i].alpha < 0.2 ? pointNotes.pop(i) : pointNotes[i].alpha - 0.1;
        ctx.globalAlpha = 1;
    }
