// Global variables
let w, h, c, ctx, activeWordList, wordCount, danger, points, multiplier, dangerColor, addNewWord, seconds;
...
function checkForMatch() {
    for (let i = 0; i < activeWordList.length; i++) {
        if (textInput.value == activeWordList[i].text) {
            points+= multiplier * activeWordList[i].text.length;                        // add the points
